...
int ants_slice_regularized_registration( itk::ants::CommandLineParser *parser )
{
  ...
  typedef itk::PolynomialConstrainedSlicewiseTranslationTransform<RealType, ImageDimension> PolyTransformType;
  typename PolyTransformType::Pointer polyTransform = PolyTransformType::New();
  polyTransform->SetPolynomialDegree( polydegree );
  polyTransform->SetIdentity();
...
typedef itk::ImageRegistrationMethodv4<FixedImageType, MovingImageType, TranslationTransformType> TranslationRegistrationType;
  ...
  RealType err = 0;
  RealType eulerparam = 1.0; // weight on new solution
  for ( unsigned int i = 0; i < transformList.size(); i++) 
    {
  ...
  typename PolyTransformType::ParametersType polyparams = polyTransform->GetParameters();
  polyparams[ 0 ] = interceptx;
  unsigned int polyoffset = polydegree + 1;
  polyparams[ polyoffset  ] = intercepty;
  for ( unsigned int i = 0 ; i < polyx.size(); i++ )
    {
    polyparams[ i + 1 ] = polyx[ i ];
    polyparams[ polyoffset + i + 1 ] = polyy[ i ];
    }
  polyTransform->SetParameters( polyparams );
  typename FixedIOImageType::IndexType index;  
  typename FixedIOImageType::PointType pointLo;  
  typename FixedIOImageType::PointType pointHi;  
  index.Fill( 0 );
  fixedImage->TransformIndexToPhysicalPoint( index, pointLo );
  index[2]=timedims-1;
  fixedImage->TransformIndexToPhysicalPoint( index, pointHi );
  polyTransform->SetZMean( pointLo[2] + (pointHi[2] - pointLo[2])/2.0  );
  double numer = A.get_column(0).max_value() - A.get_column(0).min_value();
  polyTransform->SetZRange( numer / (pointHi[2] - pointLo[2])  );
  //  polyTransform->TransformPoint(  );
  if ( verbose ) 
    {
    std::cout << "A_0min " << A.get_column(0).min_value() << "A_0max " << A.get_column(0).max_value() << std::endl; 
    std::cout << polyTransform->GetNumberOfParameters() << std::endl;
    ...
    std::cout << polyTransform->GetParameters() << std::endl;
    }
  }// if loop == 0 
  ...
  // write polynomial predicted data
    ...
    typedef itk::ResampleImageFilter<FixedIOImageType, FixedIOImageType> ResampleFilterType;
    typename ResampleFilterType::Pointer resampler2 = ResampleFilterType::New();
    resampler2->SetTransform( polyTransform );
    resampler2->SetInput( movingImage );
    resampler2->SetOutputParametersFromImage( fixedImage );
    resampler2->SetDefaultPixelValue( 0 );
    resampler2->Update();
    WriteImage<MovingIOImageType>( resampler2->GetOutput(), "temp.nii.gz"  );
