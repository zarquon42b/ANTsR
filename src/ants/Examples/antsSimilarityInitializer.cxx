/*=========================================================================
  ...
  Module:    $RSfile: antsSimilarityInitializer.cxx,v $
  Language:  C++
...
int antsSimilarityInitializerImp(int argc, char *argv[])
{
  ...
  typedef typename itk::AffineTransform<RealType,
    ImageDimension> AffineType;
  typedef typename SimilarityTransformTraits<RealType,
    ImageDimension>::TransformType SimType;
  ...
  for( unsigned int d = 0; d < ImageDimension; d++ )
    ...
    typename SimType::Pointer simsearch = SimType::New();
    ...
    gcmetric->SetMovingTransform( simsearch );
    ...
    mimetric->SetMovingTransform( simsearch );
    if( mask.IsNotNull() )
    {
    ...
    if ( whichMetric.compare("MI") == 0  ) {
      ...
      movingScales( simsearch->GetNumberOfParameters() );
   ...
   for( double ang1 = ( piover4 * (-1) ); ang1 <= ( piover4 + searchfactor ); ang1 = ang1 + searchfactor )
      {
      simsearch->SetIdentity();
      simsearch->SetCenter( trans2 );
      simsearch->SetOffset( trans );
      if( useprincaxis )
      {
      ...
      for( double ang2 = ( piover4 * (-1) );
        ...
        simsearch->SetMatrix( affinesearch->GetMatrix() );
        parametersList.push_back( simsearch->GetParameters() );
        }
      }
      if( ImageDimension == 2 )
      {
        ...
        simsearch->SetCenter( trans2 );
        simsearch->SetOffset( trans );
        typename SimType::ParametersType sparams = simsearch->GetParameters();
        sparams[1]=bestscale;
        sparams[2]=ang1;
        simsearch->SetParameters( sparams );
        parametersList.push_back( simsearch->GetParameters() );
      }
    }
    ...
    typename SimType::Pointer bestaffine = SimType::New();
...
// 'main()'
int antsSimilarityInitializer( std::vector<std::string> args, std::ostream* /*out_stream = NULL */ )
{
  // put the arguments coming in as 'args' into standard (argc,argv) format;
  // 'args' doesn't have the command name as first, argument, so add it manually;
  // 'args' may have adjacent arguments concatenated into one argument,
  // which the parser should handle
  args.insert( args.begin(), "antsSimilarityInitializer" );
...
private:
  ...
  switch( atoi(argv[1]) )
    ...
    case 2:
      {
      return antsSimilarityInitializerImp<2>(argc, argv);
      }
    case 3:
      {
      return antsSimilarityInitializerImp<3>(argc, argv);
      }
    case 4:
      return antsSimilarityInitializerImp<4>(argc, argv);
    }
  return antsSimilarityInitializerImp<2>(argc, argv);
}
} // namespace ants
